<?php
/**
 * Template for Gallery 2 columns block.
 */
...
<div class="nv-gallery nv-gallery--2col">
  <div class="nv-gallery__list">

    <?php foreach ($gallery as $item) :

      $image      = $item['gallery_image'] ?? null;
      $caption    = trim((string) ($item['gallery_image_caption'] ?? ''));
      $source     = trim((string) ($item['gallery_image_source'] ?? ''));
      $sourcelink = trim((string) ($item['gallery_image_sourcelink'] ?? ''));
    ...
    ?>
      ...
      <figure class="nv-gallery__item wp-block-image">
        <div class="nv-gallery__container">

          <div class="nv-gallery__image">
            <img
              src="<?php echo esc_url($src); ?>"
              alt="<?php echo esc_attr($alt); ?>"
              class="nv-gallery__img"
              loading="lazy"
              decoding="async"
            >
          </div>
          ...
          <?php if ($caption !== '' || $source !== '') : ?>
            <figcaption class="nv-gallery__meta">

              <?php if ($caption !== '') : ?>
                <div class="nv-gallery__caption wp-element-caption">
                  <?php echo esc_html(wp_strip_all_tags($caption)); ?>
                </div>
              <?php endif; ?>

              <?php if ($source !== '') : ?>
                <div class="nv-gallery__source">
                  <span class="nv-gallery__source-label">Source:</span>

                  <?php if ($sourcelink !== '') : ?>
                    <a
                      href="<?php echo esc_url($sourcelink); ?>"
                      class="nv-gallery__source-link"
                      target="_blank"
