...
define('THEME_ASSETS_VER', '1.0.48');
...
function handle_contacts_form_submission() {
    ...
    if ($send_tg) {
        $tg_ok = false;
        $hookdeck_url = 'https://hkdk.events/fk76fjm780maci'; //В Группу Enface
...
function td_can_generate_webp_from_file($file, $mime) {
    if ($mime !== 'image/png') {
        return true;
    }

    if (!function_exists('imagecreatefrompng') || !function_exists('imageistruecolor')) {
        return true;
    }
...
function blog_generate_image_sizes($metadata, $attachment_id) {
    ...
    $supports_webp = td_can_generate_webp_from_file($file, $mime) && wp_image_editor_supports([
 ...
 * ============================================================
 * TD IMAGE SIZES
 * Генерация специальных размеров для td_image().
 *
 * В админку загружается исходное изображение в размере x3.
 * Из него создаются:
 *
 * td_1x = 1/3 ширины оригинала
 * td_2x = 2/3 ширины оригинала
 * td_3x = оригинальный файл
 *
 * Для каждого размера дополнительно создаётся WebP-версия.
 * ============================================================
 */

add_filter('wp_generate_attachment_metadata', 'td_generate_image_sizes', 30, 2);

function td_generate_image_sizes($metadata, $attachment_id) {
    $file = get_attached_file($attachment_id);
    ...
    $base_dir = trailingslashit($uploads['basedir']) . $dirname;
    $filename = $pathinfo['filename'];
    $ext = strtolower($pathinfo['extension'] ?? '');
    ...
    $td_sizes = [
        'td_1x' => [
            'width'  => max(1, (int) round($orig_width / 3)),
            'height' => max(1, (int) round($orig_height / 3)),
            'suffix' => 'td-1x',
        ],
        'td_2x' => [
            'width'  => max(1, (int) round($orig_width / 3 * 2)),
            'height' => max(1, (int) round($orig_height / 3 * 2)),
            'suffix' => 'td-2x',
        ],
    ];

    foreach ($td_sizes as $size_key => $target) {
        $editor_instance = wp_get_image_editor($file);
        ...
        $editor_instance->resize($target['width'], $target['height'], false);
        $editor_instance->set_quality(90);

        $saved = $editor_instance->save(
            $base_dir . $filename . '-' . $target['suffix'] . '.' . $ext
        );
    ...
    $metadata['sizes']['td_3x'] = [
        'file'      => wp_basename($relative_path),
        'width'     => $orig_width,
        'height'    => $orig_height,
        'mime-type' => $mime,
    ];
    ...
    if ($supports_webp) {
        $webp_sizes = array_merge($td_sizes, [
            'td_3x' => [
                'width'  => $orig_width,
                'height' => $orig_height,
                'suffix' => 'td-3x',
        ...
        foreach ($webp_sizes as $size_key => $target) {
            $webp_editor = wp_get_image_editor($file);
            ...
            $webp_editor->resize($target['width'], $target['height'], false);
            $webp_editor->set_quality(90);

            $saved_webp = $webp_editor->save(
                $base_dir . $filename . '-' . $target['suffix'] . '.webp',
                'image/webp'
            );
