...
if ( ! function_exists( 'nv_flatten_document_tree' ) ) {
	function nv_flatten_document_tree( $parent_id, $children, &$flat ) {
		...
		foreach ( $children[ $parent_id ] as $document ) {
			$flat[] = $document;
			nv_flatten_document_tree( (int) $document->ID, $children, $flat );
		}
	}
}

if ( ! function_exists( 'nv_get_document_adjacent_links' ) ) {
	function nv_get_document_adjacent_links( $current_id ) {
		$documents = get_pages(
			array(
				'post_type'   => 'document',
				'post_status' => 'publish',
		...
		$flat = array();
		nv_flatten_document_tree( 0, $children, $flat );
...
if ( ! function_exists( 'nv_render_document_bottom_nav' ) ) {
	function nv_render_document_bottom_nav( $current_id ) {
		list( $previous_document, $next_document ) = nv_get_document_adjacent_links( $current_id );
		...
		<div class="container">
			<div class="post__wrapper">
				...
				<div class="post__content">
					...
					<article id="post-<?php the_ID(); ?>" <?php post_class( 'post-content' ); ?> itemscope itemtype="https://schema.org/Article">
						<?php if ( have_posts() ) : ?>
							<?php while ( have_posts() ) : ?>
								...
								<div class="post-text entry-content" itemprop="articleBody">
									...
									<?php echo nv_render_document_child_links( get_the_ID() ); ?>
								</div>

								<?php echo nv_render_document_bottom_nav( get_the_ID() ); ?>
							<?php endwhile; ?>
						<?php endif; ?>
					</article>
				</div>
